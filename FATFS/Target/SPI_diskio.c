...
void create_file(void)
{
  ...
  f_res = f_open(pf, "pzk.txt", FA_OPEN_ALWAYS | FA_WRITE);
...
void read_file(void)
{
  f_res = f_open(&file, "pzk.txt", FA_READ);
...
//  format_disk();  //格式化文件系统
  fileSystemInit();
// create_file();   //建立文件并写入"pzk8888888\n"
  read_file();      //读取U盘文件内容放到ReadBuffer中
}
